...
def get_data_doc_number(tipo_doc, numero_doc):
    url = 'http://localhost:8090/api/v1'
    token = 'abcxyz'
    url = '%s/%s/%s?token=%s' % (url, tipo_doc, str(numero_doc), token)
    ...
    if response.status_code == 200:
        ...
        res['message'] = "Error: Documento no encontrado"
    return res
    ...
    def update_document(self):
        ...
        if self.catalog_06_id and self.catalog_06_id.code == '1':
            ...
            if self.vat and len(self.vat) != 8:
                ...
                data = get_data_doc_number('dni', self.vat)
                if not data['error']:
                    data = data['data']
                    self.name = '%s %s %s' % (data['nombres'], data['apellidoPaterno'], data['apellidoMaterno'])
            ...
            if self.vat and len(self.vat) != 11:
                ...
                data = get_data_doc_number('ruc', self.vat)
                if data['error']:
                    raise UserError(data['message'])
                data = data['data']
                # Busca el distrito
                ditrict_obj = self.env['res.country.state']
                prov_ids = ditrict_obj.search([('name', '=', data['provincia']),
                                               ('province_id', '=', False),
                                               ('state_id', '!=', False)])
                dist_id = ditrict_obj.search([('name', '=', data['distrito']),
                                              ('province_id', '!=', False),
                                              ('state_id', '!=', False),
                ...
                self.name = data['nombreComercial'] != '-' and data['nombreComercial'] or data['razonSocial']
